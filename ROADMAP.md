...
### RAG 기반 KR 종목 뉴스 브리핑
- **배경**: 현재 KR 종목은 Finnhub이 미지원이라 뉴스 없이 일반적인 시장 맥락으로만 AI 브리핑 생성
- **개선 시**: 네이버 금융 뉴스 API 또는 빅카인즈 등에서 뉴스 크롤링 → 임베딩 → 벡터 DB 저장 → 급등 감지 시 관련 기사 검색 → Claude API에 기사 본문 주입
- **필요 구성요소**: 뉴스 수집 파이프라인, 임베딩 모델(OpenAI text-embedding 등), 벡터 DB(pgvector / Pinecone / Chroma)
- **미룬 이유**: 인프라 구성요소가 많아 독립적인 학습 주제로 다루는 게 적합. 현재 US 종목은 Finnhub 헤드라인으로 충분.
